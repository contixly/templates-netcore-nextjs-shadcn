...
preserves the approved local namespace while mapping only recognized built-in
Identity input-validation codes to the transactional
`400 validation_failed` path. Duplicate codes retain `409`; unknown or custom
result codes remain non-disclosing `500 internal_error` failures with no
persisted user/session or issued cookie.
