...
`/auth/**` redirect targets. The final Identity-validation repair preserves the
approved local namespace while mapping downstream non-duplicate Identity
validation to the existing transactional `400 validation_failed` path without
persisting a user/session or exposing provider error codes.
...
| focused local Identity-validation Application/API GREEN and covering checks                                                                                                                                                                                                                                                                                                       | PASS; typed mapping 1/1, actual API behavior 1/1, Application policy/duplicate coverage 9/9, and API creation/duplicate/Identity coverage 5/5                           |
| final `dotnet restore Template.sln && dotnet build Template.sln --no-restore && dotnet test Template.sln --no-restore`                                                                                                                                                                                                                                                            | PASS; build 0 warnings/errors, Application 19/19 and API 111/111: 130/130 total                                                                                         |
